...
class PlugAccessory {
  constructor (platform, config, homebridgeAccessory, plug) {
    ...
    CustomCharacteristic = require('./homekit')(platform.api).CustomCharacteristic;
    ...
    if (plug.supportsEmeter && this.config.addCustomCharacteristics) {
      this.log.debug('Adding CustomCharacteristics (if needed) [%s]', this.homebridgeAccessory.displayName);
    ...
    // History Service
    var FakeGatoHistoryService = require('fakegato-history')('./homekit'.homebridge);
	this.historyService = new FakeGatoHistoryService("energy", this.homebridgeAccessory, {
      storage:'fs'
    });

    var self = this;
    setTimeout(function() { addHistory(self); }, 2000);

    // Submit readings to fakegato history
    function addHistory(that) {
      that.plug.emeter.getRealtime().then((emeterRealtime) => {
      	if (emeterRealtime.power == null) {
          that.historyService.addEntry({
            time: new Date().getTime() / 1000,
            power: Math.round(emeterRealtime.power_mw / 100) / 10
          });
        } else {
          that.historyService.addEntry({
            time: new Date().getTime() / 1000,
            power: Math.round(emeterRealtime.power)
          });
      	}
      });

      // Call function every 6 minutes
      setTimeout(function() { addHistory(that); }, 6 * 60 * 1000);
      // Call function every 9:50 minutes (a new entry every 10 minutes is required to avoid gaps in the graph)
      // setTimeout(function() { addHistory(that); }, (10 * 60 * 1000) - 10000);
    }
    ...
    this.homebridgeAccessory.services.forEach((service) => {
      ...
      if (service === this.historyService.service) { return; }
